...
ddcs.log.LOG_CONTAINER_SELECTOR = '#demoLog';

ddcs.log.createLogEntryElement = function(msg) {
  ...
  return $('<tr>'
	...
	).append($('<td>').attr('class', 'from').append($('<b>').text(msg.eventType))
    ).append($('<td>').attr('class', 'details').html(msg.eventDetails)
    ).append($('<td>').attr('class', 'date').text(ddcs.log.formatDate(new Date(msg.time)))
    ).attr('class',msg.isLocal ? 'localEvent' : '');
};

ddcs.log.logDetailedEvent = function(evt, eventType, details) {
  var collaborator = ddcs.getCollaborator(evt.sessionId);
  ...
  // Collab List Added events
    ...
    eventDetails = 'From "' + evt.oldValues.join(', ') + '" to "' + evt.newValues.join(', ') + '"';
  ...
  ddcs.log.onLogAdded(logMessage);
};

ddcs.log.logEvent = function(evt, eventType) {
  var collaborator = ddcs.getCollaborator(evt.sessionId);
...
ddcs.log.formatDate = function(date) {
...
ddcs.log.onLogAdded = function(logMessage) {
  //$(ddcs.log.LOG_CONTAINER_SELECTOR).prepend(ddcs.log.createLogEntryElement(logMessage));
  //ddcs.ui.hideShowLocalEvents();
};
